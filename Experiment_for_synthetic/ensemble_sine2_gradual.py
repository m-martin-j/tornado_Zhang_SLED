...
stream_name = "sine2"
project = Project("projects/single", "ensemble_sine2_gradual_training/" + stream_name)

# 2. Generating training stream
project_path = "data_streams/ensemble_sine2_gradual_training/" + stream_name + "/"
...
stream_name = "sine2"
project = Project("projects/single", "ensemble_sine2_gradual_detection/" + stream_name)

# 2.Generating detection stream
project_path = "data_streams/ensemble_sine2_gradual_detection/" + stream_name + "/"
if not os.path.exists(project_path):
    os.makedirs(project_path)
for tm in range(repeat_count):
    stream_generator = SINE2(concept_length=5000, noise_rate=0.1, transition_length=800, num_drifts=30, random_seed=tm)
...
f = open('ensemble_sine2_gradual', 'a')
