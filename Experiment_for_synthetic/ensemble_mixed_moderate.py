...
stream_name = "mixed"
project = Project("projects/single", "ensemble_mixed_moderate_training/" + stream_name)

# 2. Generating training stream
project_path = "data_streams/ensemble_mixed_moderate_training/" + stream_name + "/"
...
stream_name = "mixed"
project = Project("projects/single", "ensemble_mixed_moderate_detection/" + stream_name)

# 2.Generating detection stream
project_path = "data_streams/ensemble_mixed_moderate_detection/" + stream_name + "/"
if not os.path.exists(project_path):
    os.makedirs(project_path)
for tm in range(repeat_count):
    stream_generator = MIXED(concept_length=5000, noise_rate=0.1, transition_length=500, num_drifts=30, random_seed=tm)
...
f = open('ensemble_mixed_moderate', 'a')
